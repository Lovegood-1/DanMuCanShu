...
from utils.general import (
    build_targets, check_img_size, non_max_suppression, apply_classifier, scale_coords, xyxy2xywh, xywh2xyxy, plot_one_box, strip_optimizer, is_ip)
...
def detect_multi_videos(video_info  ,save_img=True):
    """从上面 detect2 复制，适用于多个视频的输入

    Args:
        path_video : 所有视频及其时间. Defaults to [{'name':0721, "one":path1; "two": path2}, {'name':0722, "one":path1; "two": path2}]
        save_img (bool, optional): _description_. Defaults to True.

    Returns:
        _type_: [{"one":{"path": d:/, "info": { "fire_info"}}; "two":{"path": d:/, "info": { "fire_info"}}；} }, 
                 {'name':0722, "one":path1; "two": path2}]
    """

    """需要修改

    这个函数的目的是修改 path_video，你最好备份一下变量。
    """
    parser = argparse.ArgumentParser()
    ...
    """
    需要修改的第一处
    这里的 source 应该可以传单个视频的路径，你需要依据多个路径，循环创建 dataset，然后把检测结果放到 dict中
    """
    # Set Dataloader

    """
    需要修改的第二处：原来我们是一下处理2视频，现在不需要了，你就建立一个字典就好
        dict_ = {}  
    dict_[0] = {} # 放第一个视频的帧 以及火焰 {帧数：BBOX}
    dict_[1] = {} # 放第二个视频的帧 以及火焰 {帧数：BBOX}  
    """
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        if opt.update:  # update all models (to fix SourceChangeWarning)
            ...
            video_info =             [{'time':'20220727_111040', 
                '192.168.1.1':
